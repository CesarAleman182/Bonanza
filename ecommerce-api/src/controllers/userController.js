...
const getAllUsers = async(req, res, next)=>{
    try{
        ...
        res.status(200).json({message:'Users retrieved successfully', users, totalPages: Math.ceil(total / limit), currentPage: page, total});
    }catch(error){
        next(error);
    }
};
...
const changePassword = async (req, res, next)=>{
    try{
        ...
        res.status(200).json({
            message: 'Password was changed successufully'
        });
    }catch(error){
        next(error);
    }
...
const deactivateUser = async (req, res, next)=>{
    try{
        ...
        res.status(200).json({message: 'User deactivatied successfully'});
    }catch(error){
        next(error);
    }
};
